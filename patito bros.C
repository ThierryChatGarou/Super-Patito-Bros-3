...
if(paisaje[((y+8)-((y+8)%16))/16][((x+8)-((x+8)%16))/16]==53)
  {
  vy=vy+1.0;
  if(tecla[KEY_ALT])  //control de vuelo en el aire
    {
    vy=vy-0.25;
    }
...
void bloque_agua()  
{
if(paisaje[((y+8)-((y+8)%16))/16][((x+8)-((x+8)%16))/16]==73)  //verificando si el centro del pato esta en el agua
  {
  nadando=1;
  }
